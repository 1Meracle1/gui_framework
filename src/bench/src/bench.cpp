...
namespace bench {
    ...
    int run_benchmarks(BenchCase const* bench_cases, size_t bench_case_count) {
        if (bench_cases == nullptr && bench_case_count != 0) {
            std::fprintf(stderr, "benchmark runner received a null benchmark case array\n");
            return 1;
        }
        ...
        for (size_t index = 0; index < bench_case_count; ++index) {
            ...
            if (bench_case.fn == nullptr) {
                std::fprintf(stderr, "[bench] %s: missing function\n", bench_case.name);
                return 1;
            }
            ...
            std::printf("[bench] %s: %lld ns total, %lld ns/iteration (%u iterations)\n",
                        bench_case.name,
